...
public class BasicTeamAG extends ControlSystemSS {
    ...
    private double validateParam(Double param) {
        if (param == null || param.isNaN() || param.isInfinite()) {
            LOGGER.warning("Parámetro inválido detectado. Usando valor predeterminado.");
            return 1.0;
        }
        return param;
    }
    ...
    private Vec2 calculateAction(int mynum, Vec2 ball, Vec2 theirGoal, Vec2 ourGoal, Vec2[] teammates, Vec2[] opponents,
            PlayerParams p) {
        ...
        Vec2 dangerousOpponent = detectDangerousOpponent(opponents, ball, ourGoal);
        ...
        if (mynum == 0) {
            ...
            return actionGenericPlayer(p, ball, kickSpot, backSpot, teammates, opponents, findClosest(teammates), ourGoal);
        }
    }

    private Vec2 detectDangerousOpponent(Vec2[] opponents, Vec2 ball, Vec2 ourGoal) {
        return Arrays.stream(opponents)
                .min(Comparator.comparingDouble(opponent -> Math.sqrt(Math.pow(opponent.x - ourGoal.x, 2) + Math.pow(opponent.y - ourGoal.y, 2))))
                .orElse(DEFAULT_POSITION);
    }
     

    private Vec2 actionGoalie(PlayerParams p, Vec2 ball, Vec2 kickSpot, Vec2 goaliePos, Vec2[] teammates,
            Vec2[] opponents) {
        if (ball.r > p.disPos) {
            return goaliePos;
    ...
    private Vec2 actionGenericPlayer(PlayerParams p, Vec2 ball, Vec2 kickSpot, Vec2 backSpot, Vec2[] teammates,
            Vec2[] opponents, Vec2 closestTeammate, Vec2 ourGoal) {
        if (ball.r > p.disPos) {
            return backSpot;
        } else if (ball.r > p.disKick) {
            return (ball.r < p.kickThreshold) ? kickSpot : passToTeammate(teammates, p);
